...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    # load motion files from the wandb registry
    def load_motion_library(registry_names: str) -> list[str]:
        """Load multiple motion files from comma-separated registry names."""
        import pathlib
        import wandb
        ...
        for registry_name in registry_names.split(','):
            ...
            motion_file = str(pathlib.Path(artifact_dir) / "motion.npz")
    ...
    if len(motion_files) == 1:
        ...
        print(f"[INFO] Multi-motion training enabled with {len(motion_files)} motions")
    
    registry_name = registry_names  # For runner initialization
